...
| `fold_live.sbatch` | `sbatch tools/rails/fold_live.sbatch` | The periodic duty: merges each year's published `live.parquet` into its `items.parquet`, uploads the year then an empty live, and prints the laptop commands. The printed commands regenerate each folded year's search sidecar (`items.idx.json`, `tools/make_search_sidecar.mjs`) and upload it, because the fold makes the published sidecar stale; the explorer falls back to the footer until the new one lands. `YEARS` unset folds every year whose published live has rows (one HEAD and one footer read per year); `YEARS=2022,2026` names them. |
